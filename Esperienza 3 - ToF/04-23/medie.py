...
dT_1e2=np.loadtxt("dT1e2_100cm.txt", unpack=True )
dT_2e3=np.loadtxt("dT2e3_100cm.txt", unpack=True)
dT_1e3=np.loadtxt("dT1e3_100cm.txt", unpack=True)
...
hist1=TH1F("#Delta T tra PMT01 e PMT02", "#Delta T tra PMT01 e PMT02", num_bin, -15, 0)
for i in range(len(dT_1e2)):
	hist1.Fill(dT_1e2[i])
	
fit_function_1 = TF1("fit_function_1", "gaus(0)+gaus(3)", -10, -2)
fit_function_1.SetParameters(157, -6, 0.44, 55, -6, 1.27)
...
legend1.AddEntry("", f" Data acquisizione: 23/04/2024", "")
...
legend1.AddEntry("", f"Posizione 100cm", "")

legend1.Draw()
c1.Draw()
c1.SaveAs("dT1e2_100cm.root")

a=np.array([p1_a, p1_err_a, abs(p2_a), p2_err_a, mediana_1e2, err_mediana, mt_1e2, err_mt])
np.savetxt('medie_1e2_100cm.txt', a)
