...
dist=np.array([diff(2.5), diff(100), diff(140), diff(220), diff(262.5), diff(180)], dtype=float)
...
for i in range(10):
	...
	popt, pcov = curve_fit(retta, dist, t, sigma=dtot, absolute_sigma=False)
...
plt.errorbar(dist, t, dt, dd ,'o', color = 'blue', linestyle = ' ', ecolor='blue', elinewidth=0.8, markersize=2.5, capsize=2.5, capthick=0.8)
 
plt.xlabel('$2x_1$-d [cm]', fontdict={'family': 'serif', 'color':  'black', 'weight': 'normal', 'size': 26, })
plt.ylabel('$\Delta$T PMT01-PMT02 [ns]', fontdict={'family': 'serif', 'color':  'black', 'weight': 'normal', 'size': 26, })
plt.grid()

legenda = "Parametri del fit:\n"
legenda += "$m$ = ({:.2f} $\pm$ {:.2f})$\cdot$ 10$^{{-2}}$ ns/cm\n".format(m*100, m_err*100)
legenda += "$q$ = ({:.2f} $\pm$ {:.2f}) ns\n".format(q, q_err)
legenda += "$v_{{prop}}$ = ({:.2f} $\pm$ {:.2f}) cm/ns\n".format((1/m), (m_err*(1/m**2)))
legenda += "$\\beta$ = {:.3f} $\pm$ {:.3f}\n".format((1/m)/(30), (m_err*(1/(m**2)))/(30))
legenda += "$\chi^2$ / ndof = {:.0f} / {:d}".format(chisq,degrees_of_freedom)
plt.text(0.1, 0.9, legenda, transform=plt.gca().transAxes, fontsize=27.5, verticalalignment='top', bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.5'))

plt.legend(loc='upper left', bbox_to_anchor=(1, 1))
plt.title('Fit per la velocità di propagazione', fontdict={'family': 'serif', 'color':  'black', 'weight': 'normal', 'size': 26, })
...
plt.savefig('grafici/vel_prop.pdf', dpi=1200, bbox_inches='tight')

vel_prop=np.array([(1/m), (m_err*(1/(m**2))), m, m_err, q, q_err, pcov[0][1] ])
#np.savetxt('vel_prop.txt', vel_prop)
