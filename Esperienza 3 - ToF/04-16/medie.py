...
dT_1e2=np.loadtxt("dT1e2_140cm.txt", unpack=True )
dT_2e3=np.loadtxt("dT2e3_140cm.txt", unpack=True)
dT_1e3=np.loadtxt("dT1e3_140cm.txt", unpack=True)
"""

dT_1e2=np.loadtxt("dT1e2_min_140cm.txt", unpack=True )
dT_2e3=np.loadtxt("dT2e3_min_140cm.txt", unpack=True)
dT_1e3=np.loadtxt("dT1e3_min_140cm.txt", unpack=True)
"""
...
hist1=TH1F("#Delta T tra PMT01 e PMT02", "#Delta T tra PMT01 e PMT02", num_bin, -10, 10)
for i in range(len(dT_1e2)):
	hist1.Fill(dT_1e2[i])
	
fit_function_1 = TF1("fit_function_1", "gaus(0)", -3.5, 1.4)
fit_function_1.SetParameters(157, -1.2, 0.44, 55, -1, 1.27)
...
legend1.AddEntry("", f" Data acquisizione: 16/04/2024", "")
legend1.AddEntry(hist1, f"Histogram Entries: {entries:.0f}", "l")
legend1.AddEntry(fit_function_1, "Fit Function: Gauss + Gauss bkg", "l")
...
legend1.AddEntry("", f"Posizione 140cm", "")
legend1.AddEntry("", f"Algoritmo: metodo della soglia", "")
#legend1.AddEntry("", f"Algoritmo: metodo dei minimi", "")

legend1.Draw()
c1.Draw()
#c1.SaveAs("dT1e2_140cm.root")

a=np.array([p1_a, p1_err_a, abs(p2_a), p2_err_a, mediana_1e2, err_mediana, mt_1e2, err_mt])
#np.savetxt('medie_1e2_140cm.txt', a)
